...
function M.addMenu(playerGroup, restartHours)
    ...
    MENU_GROUP_COMMAND:New(playerGroup, "Personal Status", infoMenu, function()
        ...
        MESSAGE:New(string.format("You slung %i groups of units.", ctld.countGroupsByPlayer(playerName, coalitionNum)), 20):ToGroup(playerGroup)
        MESSAGE:New(string.format("You slung %i SAM systems.", ctld.countAASystemsByPlayer(playerName)), 20):ToGroup(playerGroup)
        MESSAGE:New(string.format("You delivered %i JTACs to the field.", ctld.countJTACsByPlayer(playerName, coalitionNum)), 20):ToGroup(playerGroup)
        MESSAGE:New(string.format("You were killed by deebix %i times.", 88), 20):ToGroup(playerGroup)
        local windDirection, windStrength = utils.getWind(point)
        MESSAGE:New(string.format("Your wind direction: %s, and strength: %s", windDirection, windStrength), 20):ToGroup(playerGroup)    
    end)
