...
--- Enumerators.
weathermark.imperial="i"
weathermark.metric="m"
...
function weathermark._OnEventMarkChange(Event)
  ...
  if Event.text~=nil and Event.text:lower():find(weathermark.keyphrase) then
    ...
    if _options.alt then
      if (_options.unitsystem and _options.unitsystem=="i") or (_options.unitsystem==nil and weathermark.unitsystem=="i") then
  ...
  if _options.laste then
    ...
    if Event.groupID > 0 then
      ...
      trigger.action.markToGroup(weathermark.markid, _report, vec3, Event.groupID, false, "")      
    else
      --weathermark.info(weathermark.id..string.format("Mark # %d added for coalition %d.", weathermark.markid, Event.coalition))
      trigger.action.markToCoalition(weathermark.markid, _report, vec3, Event.coalition, false, "")      
    end
...
function weathermark._MarkTextAnalysis(text)
  ...
  if text:lower():find(weathermark.keyphrase) then
    switch.report=true
  end
  
  if switch.report == false then  
    if text:lower():find(weathermark.keyphrase.." set") then
      switch.set=true
    elseif text:lower():find(weathermark.keyphrase.." laste") then
      switch.laste=true
    else
      weathermark.info(weathermark.id..'WARNING: NEITHER "WEATHER"/"LASTE" nor "SET" keywords specified!')
      return nil
    end
  end
  ...
  for _,keyphrase in pairs(keywords) do
    ...
    if (switch.report or switch.set) and keyphrase:lower():find("i") then
    
      switch.unitsystem="i"
      weathermark.info(weathermark.id..string.format("Keyword unit = %s", switch.unitsystem))
      
    elseif (switch.report or switch.set) and keyphrase:lower():find("m") then
