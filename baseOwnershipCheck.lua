...
function M.getAllBaseOwnership(_passedBaseName, _playerORunit, _campaignStartSetup)
    ...
    elseif _passedBaseName == "ALL" then
		...
		-- =AW=33COM this is a limitiation to use world.getAirbases(), since it requires to setup zones on every base on the map even if you are not using it, so 
		-- I decided to get bases from the json file and use those only		
        for _, base in pairs(world.getAirbases()) do
			local _baseName = base:getName()			
			if bases.ActualBasesUsedOnMap[_baseName] then										
